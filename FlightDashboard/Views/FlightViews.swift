...
struct FlightHeader: View {
    ...
    var body: some View {
        VStack {
            ...
            // Text(flight.state.rawValue)
            HStack {
                Text(flight.getState().key)
                if flight.getState() != .boarding {
                    //Text(flight.getRelativeDate(), style: .relative)
                }
                if flight.getState() == .landed {
                    Text("ago")
                }
                // Text(flight.relativeDate, style: .time)
            }
        }
    }
}
...
struct FlightFooter: View {
    ...
    var body: some View {
        VStack {
            ProgressView(value: flight.progress / 100)
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading) {
                Text("\(flight.origin.code)-\(flight.origin.name)")
...
struct ArrivingFlightView: View {
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading) {
                Text("\(flight.destination.code)-\(flight.destination.name)")
